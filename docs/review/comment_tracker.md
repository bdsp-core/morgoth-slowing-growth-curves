...
| R2-42 | figure checker | Red carries eight meanings across the set (left hemisphere, report slowing, delta-excess, visible-only-in-sleep, abnormal, anterior, experts-above-curve) and blue nearly as many | **partial** | the two that actively misled are fixed (stage colours, clean-normal grey). A full one-hue-one-concept table across all 16 figures is a larger pass and is the checker's remaining top advisory |
| R2-43 | figure checker | Three warm tones read as one colour: LENS vermillion, ON-100 rust, brick red medians | open | collapse to one warm accent so orange stays exclusive to LENS |
| R2-44 | figure checker | Quantity names differ between figures ("Relative delta (δ / total)" / "rel_delta" / "relative delta"; "TAR" / "log TAR" / "theta/alpha ratio") | open | one label string per quantity, used verbatim on every axis and colourbar |
| R2-45 | figure checker | Figure 1B/S4 and Figure S5 report per-stage n that differ tenfold for the same cohort (N2 817 vs 7,878) | done | Real and explainable: Figure 1B applies the source-appropriate rule (wake from routine, sleep from overnight, never pooled, because ratio features are not comparable across acquisition types) and Figure S5 does not. Stated in both captions. **Whether S5 should apply the same restriction is an author call** — it would change a committed supplementary figure |
| R2-46 | figure checker | Figure 2 gives Morgoth's focal AUROC without a CI while Figure S3 gives [0.83–0.97] for the same quantity | open | one-line change in `scripts/55` plus a rebuild; left for the next pass rather than made at the end of a verification cycle |
