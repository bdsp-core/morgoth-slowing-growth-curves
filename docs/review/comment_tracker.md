...
| R2-51 | figure checker | Orange carried four meanings (LENS, the ON-100 arm in S2, the normative-field box in S1, the S9 boxplot median) and was visually adjacent to the abnormal brick red, blurring "LENS" and "abnormal" across the manuscript | done | orange is now LENS's alone; abnormal moved from brick to a deep crimson that separates from it in print and in greyscale; S1, S2 and S9 restyled |
| R2-52 | figure checker | The stage palette (gold=W, blue=N2), which a reader meets over ~20 panels of Figures 1A and S5, was reused for anterior/posterior in S8B | done | topography moved to ochre/violet, outside the stage hues |
| R2-53 | figure checker | Two panel-label conventions plus four figures with none; Figure S2's 15 panels uncitable | done | one helper (`palette.panel_letter`, 10 pt bold, outer top-left) used by every producer; S2's 15 panels lettered A–O |
| R2-54 | figure checker | The four ROC figures used two tick sets, two decimal conventions (x at 2 dp against y at 1 dp in the same panel), three type sizes and two title cases | done | `palette.style_roc()` — square axes, one tick set, one decimal convention, one type ladder — applied to Figures 2, 3, S3 and S7 |
| R2-55 | figure checker | Nine different uncertainty treatments across sixteen figures, none declared | done | one convention (percentile bootstrap 95% CI, resampling the recording), stated once in a Conventions block at the head of the display-item list, with the deliberate exception (Figures 1A/S5 show normative percentiles, not CIs) named there and again in those two captions |
| R2-56 | manuscript checker | **"Undisclosed LLM labeling step"** — `gen_labels_llm.csv` sits beside the label path and `scripts/120` names `gen_class` / `p_gen_pathologic` in its column list | done | Verified: neither column survives into `report_manifest_v6.parquet` (`'gen_class' in columns` → `False`), nothing reads the CSV, and the pathologic-vs-physiologic split is the rule in `label_rederive_sap.py`. Annotated at all three places an auditor looks: the builder's column list, REPRODUCE.md, and the check that proves it |
| R2-57 | manuscript checker | §3.8's per-patient denominator (19,395) is arithmetically impossible against the cohort's 21,757 patients and 1,664 dropped recordings | done | It is drawn from the full manifest (27,524 EEGs / 23,543 patients), before the inclusion filter — a different population, so the two must not be differenced. Stated in §3.8, and `table1_sap.py` now emits both denominators |
| R2-58 | manuscript checker | The Abstract implied the norms were fitted on all 25,536 EEGs; §2.4 rejected "subsampling" while §3.2 reports a 3,000-recording subsample | done | §2.4 now distinguishes the two operations explicitly: segments are *weighted*, never subsampled; recordings *are* sampled (a seeded 3,000) precisely so the remaining 7,216 can test the centiles |
| R2-59 | manuscript checker | Direct contradiction on where "normal report + physiologic slowing" recordings go — clean-normal reference in §2.1, indeterminate in §3.1 | done | one rule stated in §2.1: clean-normal requires a normal impression **and** no slowing named at all; anything else is indeterminate and excluded from both arms |
| R2-60 | manuscript checker | "Morgoth" names both the sleep stager and the reference detector | done | **morgoth2 stager** vs **Morgoth gate**, disambiguated once in §2.3 and used consistently |
| R2-61 | manuscript checker | "spatial stability" appears in §3.4a's focal feature list but not §2.7a's; k attributed to two different splits; "band determination is near-chance" against a standard §3.7 does not apply to itself; "above most of the panel on both axes" is ON-100 only | done | all four corrected |
| R2-62 | figure checker | Grey was both the van Putten index curve and the expert operating points **inside the same axes** in Figure S3 — the method and the humans were indistinguishable | done | the van Putten family gets its own hue (dark ochre); grey is now only ever "experts / reference" |
| R2-63 | figure checker | The Fig 1B/S4-vs-S5 n discrepancy was explained in the captions, which the reviewer never sees — it reads images | done | the reason (source-appropriate vs pooled) is now printed **in** both figures, not only in the captions |
| R2-64 | Westover | **Figure 5A showed what reads as LRDA or lateralised periodic discharges**, not plain generalized slowing — a pattern reviewers would object to | done | Pin replaced: S0001119160767 (85M, N2, 4.0 SD) → **S0001116438689** (71F, N2, 4.8 SD), same marked band and same dominant stage, whose window is continuous irregular polymorphic delta–theta across all chains, symmetric, with no stereotyped repeating waveform — and whose report independently says "generalized **symmetric** theta/delta slowing". Note the existing report-text screen could not have caught the old one: its report says "occasional **periods** of generalized delta slowing", which matches no `\bperiodic\b` pattern. New tool `scripts/screen_generalized_exemplars.py` scores candidate windows for rhythmicity, narrowbandness and asymmetry to shortlist; the final choice was made by reading the traces, because the scores alone did not reproduce a clinician's judgement (recorded honestly in that script's docstring) |
