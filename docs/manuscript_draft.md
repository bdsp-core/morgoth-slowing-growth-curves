...
(median centile error 1.0 point). Against the ON-100 majority LENS reached AUROC 0.971 (generalized) and
0.921 (focal), placing 94% and 71% of experts under its curves and exceeding a foundation model and the best
published index by 0.10--0.15. On SAI-100 it matched experts for focal slowing but ranked last of three for
...
A single model trained on report data (patient-stratified, \~16k recordings) when externally validated on the ON-100 panel separated slowing from normal against the panel majority at AUROC 0.971 \[95% CI 0.933--0.997\] (generalized) and 0.921 \[0.824--0.988\] (focal), with average precision (AUPRC) 0.91 (generalized) and 0.78 (focal) against panel base rates of only 18% and 12% positive (recording-level bootstrap; **Figure 2**). This placed 94% of the 18 experts under our generalized ROC curve (89% under PR) and 71% under focal (65% under PR) (panel base rates 18/100 generalized-, 12/100 focal-positive), a majority of the panel on both axes. Against the Morgoth gate the model clearly wins on both (generalized 0.971 vs 0.853, 94% vs 11% experts under; focal 0.921 vs 0.908, 71% vs 41% experts under).
...
> **\[verify before submission: the SAI-100 numbers in this section were computed with the previous top-5 recording aggregation and have not yet been recomputed at top-20 (§2.7a). Re-running requires the SAI-100 expert/SCORE-AI workbook, which is not in the repository. Do not submit with the two external sets scored under different aggregations.\]**
...
- **Figure 2 --- Detection vs 18 experts and the foundation-model gate (ON-100).** Generalized AUROC 0.971 \[0.933--0.997\] (94% experts under); focal 0.921 \[0.824--0.988\] (71% experts under) (`figures/story/s0d_single_occasion_generalized.png`, `s0e_occasion_focal.png`; `scripts/53–55, 66`).
...
- **Figure S5 --- Stage-resolved curve bank** (rel_delta / TAR / DAR, whole-head; `figures/stage_curves/`; `scripts/111`).
...
- **Figure S3 --- van Putten indices vs LENS vs the Morgoth gate on the clean ON-100 panel** (ROC per axis, expert-majority labels). LENS is the best detector on both axes --- generalized 0.971, focal 0.921 --- beating the best hand-crafted index by +0.15 / +0.10 and edging the foundation-model gate (`figures/figs/vanputten_panel_s7.png`; `scripts/vanputten_panel_s7.py`).

- **Figure S4 --- Scalp topography of the theta/alpha ratio (TAR) by age × sleep stage** (`figures/growth_v2/topo_TAR_by_age_stage.png`; `scripts/77`). The TAR companion to Figure 1b, confirming that the regional development pattern is not specific to relative delta.
