...
The second external benchmark, SAI-100, tests transfer across countries and to an independent scoring system: 100 routine scalp EEGs from the SCORE-AI validation study \[25\], drawn from three hospitals in two countries, with SCORE-AI, morgoth, and 14 expert calls available per recording and no overlap with training or ON-100. The unchanged pipeline ran end-to-end on all 100 recordings (**Supplementary S17**).
...
- **Figure 3 --- External validation (SAI-100).** LENS vs **SCORE-AI** vs Morgoth vs the 14 individual experts, for focal and generalized slowing, against the true expert-vote majority (ground truth recomputed from the individual expert votes, see §3.4b).
