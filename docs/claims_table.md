...
|---|---|---|---|---|
| 1 | *"There is slowing."* | Morgoth gate | AUROC **0.853** vs 18-expert majority (generalized); **0.908** (focal) — canonical Figure-2 panel eval. LENS (the Morgoth-free detector) reaches **0.961 / 0.908** on the same panel (`results/story/s0d_single_model.md`, `s0e_recording_model.md`; manuscript §3.4a). *The 0.946 / 0.923 that stood here was the pre-C51 seven-region model and is superseded; the manuscript is the authority.* | **ALLOWED** — the gate's claim |
...
| 4e | *"peaking at T3 (the T3–T5 derivation)"* — **electrode-level peak** | within the asserted lobe, the homologous derivation pair with the largest LEFT−RIGHT delta asymmetry (in the asserted direction); named only when that asymmetry ≥ 0.5 log-power | agrees with the asserted side by construction; asymmetry cancels the symmetric frontal/eye-movement delta gradient that makes raw power a frontopolar attractor; fired on ~40% of focal recordings. Reports carry **no electrode field**, so there is nothing to score it against | **PROVISIONAL** — an output-granularity gain for automated reporting, one level finer than the lobe; phrased "peaking at …", never a resolved generator localization |
| 5 | *"delta-predominant"* / *"theta-predominant"* / *"mixed"* | **absolute delta/theta power dominance** = whole-head mean log(δ/θ), marginal-matched to the report band rates (`scripts/band_calibration.py`, N=9,412) | a clinician's *delta slowing* means delta power dominates the trace, which the per-band deviation z does **not** track (normal delta σ is large, so a big delta in a young brain sits at a modest z while a smaller theta excess wins z_θ − z_δ even as delta dominates). The raw log(δ/θ) ratio separates the reports' **delta-from-theta at AUROC 0.74** (vs **0.68** for the deviation axis z_θ − z_δ). The report band is **62.9% "mixed"** among the 10,244 band-naming recordings — a reader hedge that sits between the two pure bands and is only weakly separable from either — so maximising 3-way accuracy collapses to "always mixed" (0.64); **marginal-matching** (default mixed; pure call only past a report-rate-matched threshold on log(δ/θ)) reproduces the report distribution and reaches **53% at Cohen κ ≈ 0.10**, the low end of expert-vs-expert band κ (0.09–0.38) | **PROVISIONAL — at the human floor.** Report band as a **low-confidence** δ/θ/mixed gloss, calibrated to the report DISTRIBUTION (not to accuracy). Do not chase it further: κ is already at the expert floor and theta/mixed is unlearnable. The valid test is the continuous D1 dose-response, not the categorical call |
...
1. **`results/example_reports_final.md` is retired.** It emits *"Rare mild generalized mixed theta/delta
   ...
   forbidden), a frequency word (6b, forbidden), and a peak statistic (10, forbidden),
   *(Band, clause 5, was forbidden at the time this paragraph was written. It was reclassified
   **PROVISIONAL** once `scripts/band_calibration.py` showed the call can be made from absolute
   delta/theta dominance at AUROC 0.74 and marginal-matched to the report distribution, landing at
   κ ≈ 0.10 — the low end of the expert-vs-expert range, which is where it is allowed to ship, as a
   low-confidence gloss and never as a confident assertion. Clause 5 above is the authority.)*
