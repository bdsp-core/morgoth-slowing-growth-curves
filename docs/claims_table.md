...
| 5 | *"delta-predominant"* / *"theta-predominant"* / *"mixed"* | z_θ − z_δ (p90), marginal-matched to the report band rates (`scripts/band_calibration.py`, N=9,412) | the report band is **~64% "mixed"** — a reader hedge, statistically **inseparable from theta (AUROC 0.40)** and separable only from delta (0.68), so the only real signal is the **delta-vs-theta axis**. Maximising 3-way accuracy collapses to "always mixed" (0.64); **marginal-matching** (default mixed; pure call only past a report-rate-matched threshold) reproduces the report distribution and reaches **51% at Cohen κ ≈ 0.09**, the low end of expert-vs-expert band κ (0.09–0.38). When both we and the reader commit to a pure band, delta-vs-theta agreement ≈ 0.74 | **PROVISIONAL — at the human floor.** Report band as a **low-confidence** δ/θ/mixed gloss, calibrated to the report DISTRIBUTION (not to accuracy). Do not chase it further: κ is already at the expert floor and theta/mixed is unlearnable. The valid test is the continuous D1 dose-response, not the categorical call |
