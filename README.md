...
[tableone](https://github.com/tompollard/tableone). Age is from the `.mat` files; **sex is from
OMOP** (`gender_concept_id`, resolved for 12,378/12,379).
...
*Sex is well balanced (~50/50) across all groups. 18 recordings with implausible age (<0 or >120)
excluded. Controls skew ~20 yr younger than the slowing groups — an age confound the normative model
conditions on; 75+ controls are relatively sparse (n=442).*

> **Expansion in progress.** The table above is the frozen original Growth_curves cohort (n=12,379).
> As we ingest additional report-labeled long EEGs from `bdsp-opendata-*` (pilot → full wave), a
> **live Table 1** that adds the newly-ingested recordings is regenerated by
> [scripts/refresh_dashboard.py](scripts/refresh_dashboard.py) →
> [docs/table1_live.md](docs/table1_live.md), alongside a monitoring dashboard
> (`results/live_dashboard.html`) with health checks and updated figures.
