...
FIGS = {
    ...
    "FigureS5_deviation_field.png":  ([f"{STY}/s2_segment_deviation.png"], 1, "44"),
    ...
    "FigureS6_localized_focal.png":  ([f"{STY}/s0_occasion_ours_v4_focal.png"], 1, "49"),
...
def compose(out_path: Path, panels: list[str], ncols: int) -> bool:
    ...
    if scale > 1.0:
        fig.savefig(out_path, dpi=int(math.ceil(MIN_DPI * scale)), bbox_inches="tight", pad_inches=0.06, facecolor="white")
    fig.savefig(out_path.with_suffix(".pdf"), bbox_inches="tight", pad_inches=0.06, facecolor="white")   # vector; dpi n/a
    plt.close(fig)
    return True
