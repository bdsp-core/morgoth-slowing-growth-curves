...
def calibrate_threshold(gate, meta):
    """Pick P(slowing) cut maximizing Youden J vs label (normal vs focal/general)."""
    g = gate.merge(meta, on="bdsp_id").dropna(subset=["p_slowing"])
...
def main():
    ...
    thr, J = calibrate_threshold(gate, meta)
    ...
    for _, m in meta.iterrows():
        ...
        if topo in ("focal", "lateralized") and region_z:
            loc = REGION_LABEL[max(region_z, key=region_z.get)]
        else:
            loc = "generalized"
        whz = dz.loc[bid, "whole_head"] if bid in dz.index and "whole_head" in dz.columns else np.nan
        thz = tz.get(bid, np.nan)
        band = "theta slowing" if (np.isfinite(thz) and thz > (whz or 0)) else "delta slowing"
        srow = sc.loc[bid] if bid in sc.index else None
        prev = float(srow.prevalence) if srow is not None else 0.0
        f = ph.StateFinding(state="Record", prevalence=prev,
                            patient_z=max(whz if np.isfinite(whz) else 0, 0), location=loc, band=band,
                            burden=float(srow.burden) if srow is not None else 0,
                            median_abn_z=whz if np.isfinite(whz) else 0,
                            max_run_min=float(srow.longest_run_min) if srow is not None else float("nan"),
                            asymmetry_z=az_t)
        s = ph.render(f)
        if srow is not None and getattr(srow, "only_in_sleep", False):
            s = s[:-1] + ", present only during sleep."
        if srow is not None and pd.notna(srow.accentuated_stage) and srow.accentuated_stage != "W":
            s = s[:-1] + f", accentuated in {srow.accentuated_stage}."
