...
ITEMS = [
    ...
    ("Figure 4 / Table 2", "How much slowing, in the recordings MORGOTH says have slowing "
     ...
     "to be slow. FOUR groups, not three: Morgoth's two EEG-level heads (FOC_SLOWING / GEN_SLOWING) are "
     "INDEPENDENT binary sigmoids, NOT a softmax — both can fire, and 31.6% of recordings (8,691) do fire "
     "both. Focal and generalized slowing genuinely co-occur and the figure says so. Median z is a clean "
     "dose-response in every stage: neither ~-0.15 < focal-only (+0.04 to +0.25) < generalized-only (+0.16 "
     "to +0.51) < BOTH (+0.38 to +1.29). Gate operating points by Youden J on clean_pair; the report labels "
     "pick the threshold only, they do not define the groups plotted. "
