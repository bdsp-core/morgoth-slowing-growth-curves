...
SECTIONS = [
    ...
    ("Localization — region (a measurement, not a classification)",
     "The system does not perform forced-choice lobe classification; it reports the region of maximum "
     "deviation. So we show the measurement: per-lobe deviation from the age-matched clinician-normal "
     "distribution, in alert (W/N1) segments, grouped by the side the clinical REPORT named. The multi-class "
     "lobe confusion matrix has been removed — it scored a multinomial logistic regression trained to "
     "reproduce the report's region label (a classifier we do not ship), against a label that is 56% "
     "temporal and, for 17% of recordings, borrowed from a different study of the same patient.",
     [(GROWTH_V2 / "region_z_boxplots.png",
       "Top row log_delta, bottom row TAR. Left/middle: per-lobe z by group; the reported-left group is "
       "elevated in the LEFT temporal lobe and the reported-right group in the RIGHT. Right panel is the "
       "strongest evidence and is WITHIN-SUBJECT: in the same recording, the ipsilateral temporal lobe is "
       "more deviant than the contralateral one (log_delta +0.94 vs +0.55, Wilcoxon p = 2e-194; "
       "TAR +1.18 vs +0.81). No between-patient confounding is possible. NOTE the honest limit: the "
       "contralateral lobe is itself ~+0.5 to +0.8 SD above normal, and the parasagittal chain separates "
       "left-from-right almost as well as the temporal chain — so the signal is HEMISPHERIC, not lobar. "
       "We claim side; we do not claim the lobe is resolved."),
      (FIGD / "region_detection_bars.png",
       "Per-region ONE-vs-NORMAL AUROC (scripts/46_region_detection.py). NOT a trained model: the score is "
       "that lobe's age-adjusted deviation, averaged over rel_delta + DAR + TAR across the lobe's bipolar "
       "channels; positives = recordings the report localizes to that lobe, negatives = clinician-normals; "
       "bootstrap CIs. Each region is scored independently of the others. All regions 0.66-0.75 — consistent "
       "with the box plots: every lobe is elevated when the report names its side."),
      (FIGD / "generalized_ap.png",
       "Generalized slowing: anterior (FIRDA-like) vs posterior (OIRDA-like) predominance, from an "
       "anterior-minus-posterior gradient. AUROC 0.66 — modest.")]),
    ("Sleep-stage specificity",
...
CAP_MD = [("results/v4a_wake_sleep.md",
          ...
          ("results/region_z_boxplots.md",
           "★ Region as a MEASUREMENT: per-lobe deviation vs age-matched normals, within-subject "
           "ipsilateral-vs-contralateral. Replaces the forced-choice lobe confusion matrix."),
