...
def main():
    ...
    for i in range(len(lobes)):
        ...
        # anchor above the ZERO line, not above the taller mean: for frontal both means are negative, so
        # max() put the marker at the top of the less-negative bar, detached from the pair it annotates.
        ax[1].text(xx[i], max(inn[i].mean(), out[i].mean(), 0) + .035,
                   "***" if p < 1e-3 else "**", ha="center", fontsize=11)
    ...
    # headroom for the significance markers: without it the tallest one (temporal) sits at the axes top and
    # overlaps the subplot title.
    _lo, _hi = ax[1].get_ylim()
    ax[1].set_ylim(_lo, max(_hi, max(max(i.mean(), o.mean(), 0) for i, o in zip(inn, out)) + .10))
    ax[1].set_ylabel("lobe focality (vs rest of head)", fontsize=9)
    ax[1].legend(frameon=False, fontsize=7.5, loc="lower left")
    ax[1].set_title("Lobe prominence rises when the report names it", fontsize=9.5); ax[1].grid(alpha=.2, axis="y")
    fig.suptitle("Laterality tracks reported side; lobe prominence tracks reported region", fontsize=11)
    fig.tight_layout(rect=[0, 0, 1, 0.93]); fig.savefig(FIG / "s4_d2.png", dpi=300, bbox_inches="tight"); plt.close(fig)
    ...
    ax[0].set_ylabel("prevalence (mean)"); ax[0].set_title("Slowing prevalence by stage: gap persists into sleep", fontsize=9.5)
    ...
    fig.tight_layout(rect=[0, 0, 1, 0.94]); fig.savefig(FIG / "s4_d5.png", dpi=300, bbox_inches="tight"); plt.close(fig)
